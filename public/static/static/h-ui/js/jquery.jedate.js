...
;(function(root, factory) {
    ...
    jedfn.eachStrhms = function(opts,boxCell) {
        ...
        time_min_all=jet.minDate.split(" ")[0].split("-");


        select_time_all=$(".jedaul").attr("dateval").split("-");
        $(".jedateprophours").empty();
        $(".jedatepropminutes").empty();
        $(".jedatepropseconds").empty();


        hour_text = boxCell.find(".jedatehms input").eq(0).val();
        min_text = boxCell.find(".jedatehms input").eq(1).val();
        sec_text = boxCell.find(".jedatehms input").eq(2).val();
        ...
        $.each([ 24, 60, 60 ], function(i, len) {
            ...
            for (var h = 0; h < len; h++) {
                ...
                if (jet.isBool(opts.hmsLimit)) {
                    ...
                    //判断限制时间范围的状态
                    if(parseInt(select_time_all[0])==parseInt(time_min_all[0])){
                        if(parseInt(select_time_all[1])==parseInt(time_min_all[1])){
                            if(select_time_all[2]==time_min_all[2]){
                                if(minhms[0]<hour_text){
                                    if(i==0){
                                        if (h < minhms[0] || h > maxhms[0]){
                            ...
                            }else if(select_time_all[2]<time_min_all[2]){
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        boxCell.find(".jedaul li").on("click", function(ev) {
            ...
            if($(boxCell.attr(jefix)).length > 0){
                ...
                $(this).parent().attr("dateval",$(this).attr("data-ymd"));
